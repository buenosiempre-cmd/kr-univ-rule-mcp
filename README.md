...
`tools/probe`로 18개 대학을 실측(국내망 포함)해 이를 데이터로 확인했습니다.

## 실측 지형도 — 18개교 분류 완료 (2026-07, 국내망 실측)

| 분류 | 대학 | 상태 |
|---|---|---|
| 🟢 LawMaster **실증** (6) | 동국, 고려, 서울시립, 건국, 국민, 숭실 | dongguk 파서로 목록·본문 파싱 확인 |
| 🟡 LawMaster 지문 (2) | 경희, 한국외대 | 엔드포인트 변형 — 어댑터 미세조정 필요 |
| 🔵 자체 시스템 (10) | 서울대, 연세, 서강, 성균관, 한양, 중앙, 홍익, 세종, 단국, 이화 | 전용 어댑터 필요 |
...
src/             [예정] MCP 서버 본체 — 멀티 어댑터 (Phase 1)
  adapters/
...
1. **Phase 1** — LawMaster 어댑터: 실증 6개교 즉시 지원 + 경희·외대 변형 흡수
